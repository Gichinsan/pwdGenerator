 ...
 *
 */
package de.gichinsan.pwdGenerator.utils;
...
public class PasswordGenerator {
    ...
    public String passwordGenerator(int slFlag, int scFlag, int dFlag,
        ...
        for (int i = 0; i < minlenght; i++) {
            ...
            sPWD.append(simplePwd[iNumber]);

            if (slFlag == 1) {

                iNumber = (int) (Math.random() * ((25 - 1) + 1)) + 1;
                sPWD.append(smallLetters[iNumber]);
            }

            if (dFlag == 1) {

                iNumber = (int) (Math.random() * ((8 - 1) + 1)) + 1;
                sPWD.append(digits[iNumber]);
            }

            if (scFlag == 1) {

                iNumber = (int) (Math.random() * ((9 - 1) + 1)) + 1;
                sPWD.append(specialCharacters[iNumber]);
            }
        }
        sPWD = new StringBuilder(sPWD.substring(0, minlenght));
        return sPWD.toString();
    }
}
